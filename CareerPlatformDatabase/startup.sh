...
echo ""
echo "Optional: Web database visualizer (Node.js) is disabled by default to keep the database container minimal."
echo "If you want to run it locally, follow these steps:"
echo "  1) cd db_visualizer"
echo "  2) npm ci"
echo "  3) source postgres.env"
echo "  4) DB_VISUALIZER_ENABLED=1 node server.js --host 0.0.0.0 --port 3070"
echo ""
echo "Note: If dependencies are missing or DB_VISUALIZER_ENABLED is not set, the script falls back to a lightweight"
echo "      health endpoint that does not require express, so container startup will not fail."
